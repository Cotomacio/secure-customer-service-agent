#!/usr/bin/env bash
# Bucket-level grant for Stage 4's Ada (mirrors Stage 1).
set -euo pipefail
...
if [[ -z "${AGENT_IDENTITY:-}" ]]; then
  : "${ORG_ID:?Set ORG_ID OR AGENT_IDENTITY directly from deploy.py output}"
...
echo "Granting Stage 4 Ada read access to $BUCKET"
