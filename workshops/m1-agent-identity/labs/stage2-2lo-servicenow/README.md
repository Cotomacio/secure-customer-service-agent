...
| **Credential shape depends on the connector type** | `agent/tools.py` body | The `credential` ADK injects is an `AuthCredential` object whose contents differ by connector type: **For 2LO / 3LO** the bearer is at `credential.http.credentials.token` (render as `f"{http.scheme.title()} {token}"` in the `Authorization` header). **For API key** the secret is at `credential.http.additional_headers["X-API-Key"]` (or `X-GOOG-API-KEY`) with `credentials.token=None`. Stage 2 uses the first form; Stage 4 uses the second. |
